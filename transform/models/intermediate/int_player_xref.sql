...
-- Pick best match per FBref player per season (above threshold)
best_matches as (
    select
        *,
        row_number() over (
...
final as (
    select
        ...
        afp.fbref_player_id,
        coalesce(o.understat_player_id, m.understat_player_id) as understat_player_id,
        afp.league,
        afp.season,
        m.confidence,
        m.name_score,
        ...
        case
            ...
            when m.fbref_player_id is not null then m.match_status
            else 'unmatched'
        end as match_status
    from all_fbref_players afp
    left join matched m
        on afp.fbref_player_id = m.fbref_player_id
        and afp.season = m.season
